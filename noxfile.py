...
def publish_package(session: nox.Session):
    ...
    session.run("twine", "upload", "dist/*", "--repository", "aboto3")
...
def unit_tests_current_python(session: nox.Session):
    ...
    session.run("pytest", "-vvv", "--cov=src/aboto3", "--cov-report", "html", "tests/unit")
...
def unit_tests(session: nox.Session):
    ...
    dev_venv_setup(session=session)
    session.run("coverage", "erase")
    session.run("pytest", "-vvv", "--cov=src/aboto3", "--cov-report", "term-missing", "tests/unit")
